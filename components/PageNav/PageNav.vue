<template>
  <div v-scroll="handleScroll" :class="{ show: show }" class="page-nav">
    <nav class="section-nav">
      <a v-for="(item, index) in menuList" :key="index" v-smooth-scroll="{ offset: -100 }"
        :style="{ top: 30 * (menu.length - item.id) + 'px' }" :href="'#' + item.url"
    ...
    <v-tooltip :nudge-top="25" location="left">
      <template #activator="{ props }">
        <v-btn v-smooth-scroll icon size="large" class="fab anchor-link scrollactive-item" href="#home" color="secondary"
          v-bind="props">
          <v-icon class="icon">
            mdi-arrow-up
          </v-icon>
